...
Tetromino CreateTetromino(TetrominoType type) {
    ...
    t.type = type;
    switch (type) {
        case I: t.blocks = { {0,0}, {1,0}, {-1,0}, {-2,0} }; t.color = SKYBLUE; break;
        case O: t.blocks = { {0,0}, {1,0}, {0,1}, {1,1} }; t.color = YELLOW; break;
        case T: t.blocks = { {0,0}, {-1,0}, {1,0}, {0,1} }; t.color = PURPLE; break;
    }
    return t;
}
...
int main() {
    ...
    Sound sfxGameOver = LoadSound("Sounds/game_over.mp3");
    ...
    while (!WindowShouldClose()) {
        ...
        if (currentState == MENU) {
            ...
            if (IsKeyPressed(KEY_LEFT)) {
                current.Move({ -1, 0 });
                if (CheckCollision(current)) current.Move({ 1, 0 });
            }
